...
(defun transpiler-pass-1st-half (tr forms)
  (with (e nil)
    (dolist (x forms e)
	  (setf e (append e
        (list (funcall
...
(defun transpiler-pass-complete (tr forms)
  (transpiler-pass-2nd-half tr (transpiler-pass-1st-half tr forms)))
...
;		  (if fun
;			  (assoc-update x
;		  	  				(expanded (funcall #'transpiler-pass-1st-half
...
(defun transpiler-sight (tr forms)
  (transpiler-pass-1st-half tr forms))
  ...
  (unless (eq t (transpiler-unwanted-functions tr))
    ...
    (with (w nil
	  ...
	  (while (not (equal w n)) nil
        (transpiler-wanted tr #'transpiler-pass-1st-half (transpiler-wanted-functions tr))
	    (setf w n
			  n (transpiler-wanted-functions tr)))))
