...
(c-define-compiled-literal c-compiled-string (x string)
  ...
  :init-maker (tregc_add_unremovable (trestring_get (%%native (%transpiler-string ,x)))))

(c-define-compiled-literal c-compiled-symbol (x symbol)
  :maker ($ 'tresymbol_compiled_ x (? (keyword? x) '_keyword ""))
  :init-maker (tregc_add_unremovable (treatom_get (%%native (%transpiler-string ,(symbol-name x)))
			                                      ,(? (keyword? x)
				                                      'tre_package_keyword
				                                      'treptr_nil))))
