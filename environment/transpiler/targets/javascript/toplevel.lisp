...
(fn js-sections-before-import ()
  (. (. '*js-core0* (read-from-string *js-core0*))
     (& (not (configuration :exclude-core?))
         (+ (list (. '*js-core* (read-from-string *js-core*)))
            (& (assert?)
               (list (. '*js-core-debug-print* (read-from-string *js-core-debug-print*))))
            (list (. '*js-core1* (read-from-string *js-core1*))
                  (. 'js-core-stream (read-from-string (js-core-stream))))
            (& (eq :nodejs (configuration :platform))
               (list (. 'js-core-nodejs (read-from-string (js-core-nodejs)))))
