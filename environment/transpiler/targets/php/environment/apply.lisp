...
(dont-obfuscate apply call function_exists call_user_func)

(defun apply (&rest lst)
  (with (fun lst.
         l (last .lst)
         args (%nconc (butlast .lst) l.)
         expander-name (%%%string+ "treexp_" fun))
    (when-debug
	...
	(aif (function_exists expander-name)
	     (call_user_func expander-name (%transpiler-native "array (" args ")"))
         (call_user_func fun (list-array args)))))
